...
fun findDestructuringDeclarationUsages(componentFunction: PsiMethod, scope: SearchScope, consumer: Processor<PsiReference>) {
    ...
    val descriptor = ktDeclarationTarget.resolveToDescriptor() as? CallableDescriptor ?: return
    ...
    Processor(dataType, ktDeclarationTarget, scope, consumer).run()
}
        ...
        for (classToSearch in classesToSearch) {
            ReferencesSearch.search(classToSearch).forEach(Processor { reference ->
                if (!processDataClassUsage(reference)) {
                    ...
                    TODO("Unsupported reference: '${element.text}' in ${element.containingFile.name}")
                }
                else {
                    true
                }
